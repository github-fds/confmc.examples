...
(
   ...
   `include "gpif2slv_trx_ahb_tasks.v"
   //---------------------------------------------------------------------------
   reg done=1'b0;
   integer error;
   integer idx, idy, idz;
...
if (TEST_GPIN_OUT) begin
       gpout = 16'h15;
       ahb_write_gpout(gpout);
       ahb_read_gpin(fiq, irq, gpin);
       $display("%m fiq=%b irq=%b gpin=0x%04X gpout=0x%04X",
                  fiq, irq, gpin, gpout);
end
       //-----------------------------------------------------------------------
if (TEST_SINGLE0) begin
       dataW = 32'h12345678;
       ahb_write(32'h104, 4, dataW);
       ahb_read (32'h104, 4, dataR);
...
if (TEST_BURST1) begin
       ...
       ahb_write_burst(32'h104, 4, 1);
       ahb_read_burst (32'h104, 4, 1);
...
if (TEST_BURST4) begin
       ...
       ahb_write_burst(32'h104, 4, 4);
       ahb_read_burst (32'h104, 4, 4);
...
if (TEST_BURST8) begin
       ...
       ahb_write_burst(32'h104, 4, 8);
       ahb_read_burst (32'h104, 4, 8);
...
if (TEST_BURST16) begin
       ...
       ahb_write_burst(32'h104, 4, 16);
       ahb_read_burst (32'h104, 4, 16);
...
if (TEST_BURST_ALL) begin
       random_seed = 7;
       for (idz=4; idz<=16; idz=idz*2) begin // burst-length
       for (idy=idz; idy<=(idz*4); idy=idy*2) begin // num of bursts
            for (idx=0; idx<idz; idx=idx+1) begin // fill data to write
                 burst_dataW[idx] = $random(random_seed);
            end
            ahb_write_burst(32'h004, 4, idz);
            ahb_read_burst (32'h004, 4, idz);
            error = 0;
            for (idx=0; idx<idz; idx=idx+1) begin // fill data to write
       ...
       //-----------------------------------------------------------------------
if (TEST_BURST_INC) begin
       random_seed = 7;
       for (idy=16; idy<=128; idy=idy+1) begin // num of bursts
          //dataW = 32'h4433_2211;
            for (idx=0; idx<idy; idx=idx+1) begin // fill data to write
                 burst_dataW[idx] = $random(random_seed);
               //burst_dataW[idx] = dataW;
               //dataW = dataW + 32'h1111_1111;
            end
            ahb_write_burst(32'h004, 4, idy);
            ahb_read_burst (32'h004, 4, idy);
            error = 0;
            for (idx=0; idx<idy; idx=idx+1) begin // fill data to write
                 if (burst_dataW[idx]!==burst_dataR[idx]) begin
                     error = error + 1;
            ...
            if (error!=0) begin
                $display("%m ERROR %d-leng burst %d errors", idy, error);
            end else begin
                $display("%m OK    %d-leng burst OK", idy);
            end
       end
end
       //-----------------------------------------------------------------------
