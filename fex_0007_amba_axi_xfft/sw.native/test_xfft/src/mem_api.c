...
int write_burst( unsigned int  addr // starting address
               , unsigned int *buff // data
               , unsigned int  size // size of each element of buff
...
int
MemTestAddWr(unsigned int saddr, unsigned int depth) {
   unsigned int i;
...
int
MemTestAddRr(unsigned int saddr, unsigned int depth) {
...
MemTestRAW(unsigned int saddr, unsigned int depth, unsigned int size) {
   unsigned int i, wd, rd, ex, mask, err;
   unsigned int send;
...
int MemTestBurstRAW(unsigned int saddr, unsigned int depth, unsigned int leng) {
   unsigned int i, j, err, size;
   unsigned int send;
...
#if 0
   dataW = NULL;
   if ((dataW = (unsigned int*)malloc(leng*4))==NULL) {
      printf("cannot alloca memory\n");
   }
   dataR = NULL;
   if ((dataR = (unsigned int*)malloc(leng*4))==NULL) {
      printf("cannot alloca memory\n");
   }
#endif
   size = 4;
