...
__date__       = "2019.04.15"
__description__= "AXI_MEM2STREAM with CON-FMC AMBA BFM"

#-------------------------------------------------------------------------------
class axi_mem2stream:
      # bit position
      M2S_ctl_en    =31
      M2S_ctl_ip    =1
      M2S_ctl_ie    =0
      ...
      # bit mask
      M2S_ctl_en_MSK    =(1<<M2S_ctl_en)
      M2S_ctl_ip_MSK    =(1<<M2S_ctl_ip)
      M2S_ctl_ie_MSK    =(1<<M2S_ctl_ie)
      
      M2S_num_go_MSK    =(1<<M2S_num_go   )
      M2S_num_busy_MSK  =(1<<M2S_num_busy )
      M2S_num_done_MSK  =(1<<M2S_num_done )
      M2S_num_cont_MSK  =(1<<M2S_num_cont )
      M2S_num_chunk_MSK =(0xFF<<M2S_num_chunk)
      M2S_num_bnum_MSK  =(0xFFFF<<M2S_num_bnum )

      def __init__(self, hdl, bfm, addr=0x00010000):
          """hdl: con_Handle_t
             bfm: BFM modeule
      ...
      def enable( self, en, ie ):
          value = [0]
          if (en!=0): value[0] |= self.M2S_ctl_en_MSK
          if (ie!=0): value[0] |= self.M2S_ctl_ie_MSK
      ...
      def set( self
          ...
          value[0] = 0
          if (go  !=0): value[0] |= self.M2S_num_go_MSK
          if (cont!=0): value[0] |= self.M2S_num_cont_MSK
          value[0] |= (chunk<<self.M2S_num_chunk)&self.M2S_num_chunk_MSK
          value[0] |= (packet<<self.M2S_num_bnum)&self.M2S_num_bnum_MSK
          self._bfm.BfmWrite(self._hdl, self._NUM, value);
          if (cnum==0): return 0
          num=0;
          while True:
               self._bfm.BfmRead(self._hdl, self._NUM, value);
               if (value[0]&self.M2S_num_go_MSK)==0: break
               num += 1
