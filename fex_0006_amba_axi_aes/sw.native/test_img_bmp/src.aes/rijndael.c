...
int aes_encrypt( AES_CYPHER_T mode
               , uint8_t *cyper
               , uint8_t *plain
               , int len
               , uint8_t *ekey )
      ...
      //printf("Encrypting block at %u ...\n", i);
        ...
        for (nr = 0; nr <= g_aes_rounds[mode]; nr++) {
          ...
          //aes_dump("input", s, 4 * g_aes_nb[mode]);
            ...
            /* do AddRoundKey */
            aes_add_round_key(mode, s, ekey, nr);
...
int aes_encrypt_org(AES_CYPHER_T mode, uint8_t *data, int len, uint8_t *key)
{
    uint8_t w[4 * 4 * 15] = {0}; /* round key */
    uint8_t s[4 * 4] = {0}; /* state */
...
{
    return aes_encrypt_org(mode, data, len, key);
}
...
int aes_decrypt( AES_CYPHER_T mode
               , uint8_t *plain
               , uint8_t *cyper
               , int len
               , uint8_t *ekey)
...
int aes_decrypt_org(AES_CYPHER_T mode, uint8_t *data, int len, uint8_t *key)
{
    uint8_t w[4 * 4 * 15] = {0}; /* round key */
    uint8_t s[4 * 4] = {0}; /* state */
...
{
    return aes_decrypt_org(mode, data, len, key);
}
...
void aes_cypher_128_test()
{
    ...
    aes_encrypt_org(AES_CYPHER_128, buf, sizeof(buf), key);

    printf("\nAES_CYPHER_128 decrypt test case:\n");
    printf("Input:\n");
    aes_dump("data", buf, sizeof(buf));
    aes_dump("key ",  key, sizeof(key));
    aes_decrypt_org(AES_CYPHER_128, buf, sizeof(buf), key);
}
...
void aes_cypher_192_test()
{
    ...
    aes_dump("data", buf, sizeof(buf));
    aes_dump("key ",  key, sizeof(key));
    aes_encrypt_org(AES_CYPHER_192, buf, sizeof(buf), key);
   
    printf("\nAES_CYPHER_192 decrypt test case:\n");
    printf("Input:\n");
    aes_dump("data", buf, sizeof(buf));
    aes_dump("key ",  key, sizeof(key));
    aes_decrypt_org(AES_CYPHER_192, buf, sizeof(buf), key);
}
...
void aes_cypher_256_test()
{
    ...
    aes_dump("data", buf, sizeof(buf));
    aes_dump("key ",  key, sizeof(key));
    aes_encrypt_org(AES_CYPHER_256, buf, sizeof(buf), key);
   
    printf("\nAES_CYPHER_256 decrypt test case:\n");
    printf("Input:\n");
    aes_dump("data", buf, sizeof(buf));
    aes_dump("key ",  key, sizeof(key));
    aes_decrypt_org(AES_CYPHER_256, buf, sizeof(buf), key);
}
